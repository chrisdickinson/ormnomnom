...
class PrivateAPI {
  ...
  contributeColumns (prefix, filter) {
    ...
    for (var key in this.ddl) {
      ...
      if (!filter(`${prefix}.${this.ddl[key].name}`)) {
        continue
      }
      out.push(`
...
function createDDL (ddl) {
  ...
  for (var key in ddl) {
    if (ddl[key][fkFieldSym]) {
      ...
      createFK(key, info.model, out, col, info.nullable || false)
      ...
      out[col] = createCol(col, info.validator || joi.any())
    } else {
...
function createFK (name, cls, ddlTarget, localColumn, remoteColumn, nullable) {
  ddlTarget[name] = {
    isFK: true,
    name: name,
    column: localColumn,
