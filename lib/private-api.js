...
class PrivateAPI {
  ...
  createValuesTransformer (values) {
    ...
    // we have access to Column#prepJSData().
    ...
    return (row, push) => {
      push(values.reduce((lhs, rhs) => {
        ...
        current[bits[0]] = specToColumn[rhs].prepJSData(row[`${this.tableName}.${rhs}`])
        return lhs
      }, {}))
    }
  }
  ...
  createValuesListTransformer (values) {
    ...
    values = values.map(spec => {
      if (spec.includes('.')) {
        ...
        if (dao && dao.ddl[bits[0]]) {
          const column = dao.ddl[bits[0]]
          return (row, push) => push(column.prepJSData(row[`${this.tableName}.${spec}`]))
        }
      }

      if (spec in this.ddl) {
        return (row, push) => push(this.ddl[spec].prepJSData(row[`${this.tableName}.${spec}`]))
      }
