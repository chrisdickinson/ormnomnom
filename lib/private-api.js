  ...
  createValuesTransformer (values) {
    ...
    return (row, push) => {
      push(values.reduce((lhs, rhs) => {
        ...
        current[bits[0]] = specToColumn[rhs].decode(row[`${this.tableName}.${rhs}`])
        return lhs
      }, {}))
    }
  }
  ...
  createValuesListTransformer (values) {
    ...
    values = values.map(spec => {
      if (spec.includes('.')) {
        ...
        return (row, push) => push(column.decode(row[`${this.tableName}.${spec}`]))
      }

      if (spec in this.ddl) {
        return (row, push) => push(this.ddl[spec].decode(row[`${this.tableName}.${spec}`]))
      }
