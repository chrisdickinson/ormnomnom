...
function materialize (queryset) {
  ...
  function buildQuery (data, where, db) {
    ...
    switch (action) {
      case UPDATE:
        return queryCount(db, values, buildUpdate(
          queryset[daoSym], db, data, where, slice, order, values))
      case DELETE:
        return queryCount(db, values, buildDelete(
          queryset[daoSym], db, where, slice, order, values))
      case SELECT:
        return queryMap(db, values, buildSelect(
          queryset[daoSym], db, where, slice, order, values))
      case COUNT:
        return queryMap(db, values, buildCount(
          queryset[daoSym], db, where, slice, order, values))
      case INSERT:
        return queryMap(db, values, buildInsert(
...
function buildSelect (dao, db, where, bounds, order, values, onlyFields) {
  ...
  for (var join of getJoins(dao, where, seenJoin)) {
    pfx.set(join.to, pfx.get(join.from) + '.' + join.on)
    joinClause.push(`LEFT JOIN "${join.to.tableName}" ON (
      "${join.from.tableName}"."${join.on}" = 
...
function buildInsert (dao, db, data) {
  var keys = []
  var col = null
  var numbers = []
  for (var key in data) {
  ...
  return `
    ...
    RETURNING ${dao.getColumns(dao.tableName)}
  `
}
