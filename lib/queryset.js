...
function buildUpdate (dao, db, data, where, bounds, order, values) {
  ...
  var onlyFields = [`"${dao.tableName}"."${dao.primaryKeyName}"`]
  const mainClause = `
...
function buildDelete (dao, db, where, bounds, order, values) {
  const onlyFields = [`"${dao.tableName}"."${dao.primaryKeyName}"`]
...
function buildCount (dao, db, where, bounds, order, values) {
  var onlyFields = [`COUNT(*) AS "${dao.tableName}.count"`]
...
function buildSelect (dao, db, where, bounds, order, values, onlyFields) {
  var joinClause = []
  var selectClause = [dao.contributeColumns(dao.tableName)]
  ...
  var addJoin = join => {
    pfx.set(join.to, pfx.get(join.from) + '.' + join.on.fieldName)
    joinClause.push(`LEFT JOIN "${join.to.tableName}" ON (
      "${join.from.tableName}"."${join.on.name}" =
      "${join.to.tableName}"."${join.to.primaryKeyName}"
    )`)
  }
  ...
  for (var join of getJoins(dao, where, seenJoin)) {
    ...
    if (!seenSelect.has(join.to)) {
      selectClause.push(join.to.contributeColumns(pfx.get(join.to)))
    }
