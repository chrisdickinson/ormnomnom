...
function materialize (queryset, destination) {
  ...
  for (var xs of iterate(queryset)) {
    ...
    if (xs._distinct) {
      distinct = distinct.concat(xs._distinct)
    }
  ...
  var distinctSet = new Set(distinct)
  ...
  return Promise.join(
    ...
    const runner = buildQueryRunner(data, where)
    const getConnection = queryset[daoSym].getConnection()

    // the runner handles releasing the connection.
    return getConnection.then(runner)
  })

  function buildQueryRunner (data, where) {
    var result = null
    var values = []
    ...
    switch (action) {
      ...
      case SELECT:
        ...
        result = buildSelect(
          dao, where, slice, order, values, queryset._columns, distinctSet)
        break
      case COUNT:
        destination = destination || queryMap
        result = buildCount(
    ...
    return pair => {
      return destination(pair.connection, values, result, pair.release)
    }
  }
}
...
function buildSelect (dao, where, bounds, order, values, onlyFields, distinctSet) {
...
${column.sqlName} AS "${column.outputName}"
  ...
  if (distinctSet && distinctSet.size) {
    builder.addDistinctExpression(distinctSet)
  }
  ...
  return `
    SELECT${
      builder.distinctExpressions.length
      ? ' DISTINCT ON (' + builder.distinctExpressions.map(
        xs => xs.sqlName
        ) + ')'
      : ''} ${selectClause.join(', ')} FROM
    "${dao.tableName}" "${builder.targetTableName}"
    ${joinClause.join(' ')}
    ${whereClause}
