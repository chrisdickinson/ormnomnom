...
function materialize (queryset, destination) {
  ...
  for (var xs of iterate(queryset)) {
    ...
    distinct = distinct || xs._distinct
    if (xs._filter) {
  ...
  function buildQueryRunner (data, where) {
    ...
    switch (action) {
      ...
      case SELECT:
        ...
        result = buildSelect(
          dao, where, slice, order, values, queryset._columns, distinct)
        break
      case COUNT:
        destination = destination || queryMap
        result = buildCount(
...
function buildSelect (dao, where, bounds, order, values, onlyFields, isDistinct) {
...
${column.sqlName} AS "${column.outputName}"
  ...
  return `
    SELECT${isDistinct ? ' DISTINCT' : ''} ${selectClause.join(', ')} FROM
    "${dao.tableName}" "${builder.targetTableName}"
    ${joinClause.join(' ')}
    ${whereClause}
