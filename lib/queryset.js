  ...
  function buildQuery (data, where, db, release) {
    ...
    switch (action) {
      case UPDATE:
        ...
        result = buildUpdate(
          dao, db, data, where, slice, order, values)
        break
      case DELETE:
        destination = destination || queryCount
        result = buildDelete(
          dao, db, where, slice, order, values)
        break
      case SELECT:
        destination = destination || queryMap
        result = buildSelect(
          dao, db, where, slice, order, values, queryset._columns)
        break
      case COUNT:
        destination = destination || queryMap
        result = buildCount(
...
function buildSelect (dao, db, where, bounds, order, values, onlyFields, rawFields) {
  ...
  var whereClause = '(' + where.map(clause => {
    ...
    return mapped.sql === '()' ? null : mapped.sql
...
function getColInfo (dao, colspec) {
  ...
  if (!(bits[j] in curDao.ddl)) {
    throw new Error(
      `"${bits[j]}" is not a valid column for ${curDao.InstanceCls.name}\n
  (from "${bits.join('.')}", available cols: "${
   Object.keys(curDao.ddl).join('", "')}")`
    )
  }
  return {
